...
def integrate_data(gwas_results_df, chr_col_name, snp_col_name, pval_col_name,
    ...
    common_df['nlogp'] = -np.log10(common_df[pval_col_name])

    common_df['L2 Score Bin'] = (pd.qcut(common_df['L2'], q=50, labels=False) + 1)

    score_bins = common_df[['L2','nlogp','L2 Score Bin']].groupby('L2 Score Bin').mean()

    return score_bins, len(common_indices)

def ld_score_regression(l2_score, nlogp, sample_size, num_snp, outpath):
    ...
    a = intercept / sample_size
    h2 = slope * num_snp / sample_size

    ax=sns.scatterplot(x=l2_score, y=nlogp,hue=weights,palette='coolwarm')
